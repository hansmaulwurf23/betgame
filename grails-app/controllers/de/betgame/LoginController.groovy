...
class LoginController {
	...
	static navigation = [
		'*': [
			visible: {!(SpringSecurityUtils.getPrincipalAuthorities() - 'ROLE_ANONYMOUS') && session.zg == 'home'},
			order: 1,
			linkclasses:['login'],
			action: 'casauth',
			subentries: [
				authfail: [visible:{false}],
				denied: [visible:{false}],
				ajaxSuccess: [visible:{false}],
				ajaxDenied: [visible:{false}],
				index: [visible:{false}],
				full: [visible:{false}],
				]
			],
	...
	def index = {
		if (springSecurityService.isLoggedIn()) {
			...
			redirect action: casauth, params: params
		}
	}
